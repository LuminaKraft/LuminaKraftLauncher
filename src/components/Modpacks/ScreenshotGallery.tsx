...
const ScreenshotGallery: React.FC<ScreenshotGalleryProps> = ({ images, modpackName }) => {
  ...
  return (
    <>
      ...
      <div 
        className={`bg-dark-800 rounded-xl p-6 border border-dark-700 mb-8 transition-all duration-200 ${
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
          <div>
            ...
            <p className="text-dark-400 text-sm">{images.length} {t('modpacks.imagesAvailable')}</p>
          </div>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center sm:justify-end space-x-3">
              <button
                ...
                className={`w-10 h-10 bg-dark-700 hover:bg-lumina-600 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg flex items-center justify-center transition-colors duration-150 ${
        ...
        <div className={`grid gap-4 ${(() => {
          const currentPageImages = images
          ...
          {images
            ...
            .map((image, index) => {
              ...
              return (
                <div
                  key={actualIndex}
                  className={`group relative aspect-video bg-dark-700/50 rounded-lg overflow-hidden cursor-pointer transition-all duration-200 ${
                ...
                >
                  ...
                  <div className={`absolute inset-0 bg-black/50 opacity-0 transition-opacity duration-200 flex items-center justify-center ${
                    getAnimationClass('', 'group-hover:opacity-100')
                  }`}>
      ...
      {imageModalOpen && selectedImageIndex !== null && (
        <div 
          className={`fixed inset-0 z-[9999] flex items-center justify-center p-6 ${
            getAnimationClass('backdrop-blur-md transition-all duration-200 ease-out', '')
          } ${
            imageModalAnimating 
        ...
        >
          ...
          <button
            onClick={closeImageModal}
            className={`absolute top-4 right-4 md:top-6 md:right-6 z-[10000] w-10 h-10 bg-white/15 backdrop-blur-sm text-white/90 rounded-full flex items-center justify-center border border-white/30 ${
              getAnimationClass('hover:bg-white/25 hover:text-white transition-all duration-300', '')
            } ${
              imageModalAnimating 
          ...
          {images.length > 1 && (
            <>
              <button
                ...
                className={`absolute left-4 md:left-6 top-1/2 -translate-y-1/2 z-[10000] w-12 h-12 bg-white/15 backdrop-blur-sm text-white/80 rounded-full flex items-center justify-center border border-white/30 ${
                  getAnimationClass('hover:bg-white/25 hover:text-white transition-all duration-300', '')
                } ${
                  imageModalAnimating 
              ...
              <button
                ...
                className={`absolute right-4 md:right-6 top-1/2 -translate-y-1/2 z-[10000] w-12 h-12 bg-white/15 backdrop-blur-sm text-white/80 rounded-full flex items-center justify-center border border-white/30 ${
                  getAnimationClass('hover:bg-white/25 hover:text-white transition-all duration-300', '')
                } ${
                  imageModalAnimating 
                    ? 'opacity-100 scale-100' 
          ...
          <div 
            className={`relative max-w-[75vw] max-h-[75vh] flex items-center justify-center ${
              ...
              imageModalAnimating 
                ? (isChangingImage ? 'scale-95 opacity-50' : 'scale-100 opacity-100')
                : 'scale-90 opacity-0'
            }`}
            style={getAnimationStyle({})}
          ...
          {/* Image counter */}
          {images.length > 1 && (
            <div className={`absolute bottom-6 left-1/2 -translate-x-1/2 z-[10000] px-4 py-2 bg-black/40 backdrop-blur-sm text-white text-sm rounded-full border border-white/20 ${
